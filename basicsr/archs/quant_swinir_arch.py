...
class VQ_SwinIR(nn.Module):
    ...
    def __init__(self,
        ...
        self.quant_dim = 1024
        self.before_quant_conv = nn.Conv2d(embed_dim, self.quant_in_dim, 1)
        ...
        #self._conv_trans_1 = nn.ConvTranspose2d(in_channels=self.quant_in_dim * 2,
        #                                        out_channels=self.quant_in_dim,
        #                                        kernel_size=4,
        #                                        stride=2, padding=1)
        #self._conv_trans_2 = nn.ConvTranspose2d(in_channels=self.quant_in_dim,
        #                                        out_channels=embed_dim,
        #                                        kernel_size=4,
        #                                        stride=2, padding=1)
        #self._conv_trans_3 = nn.ConvTranspose2d(in_channels=embed_dim,
        #                                        out_channels=embed_dim,
        #                                        kernel_size=4,
        #                                        stride=2, padding=1)
    ...
    def forward_features(self, x):
        ...
        '''
        # Upsample to the same as input
        N, C, xH, xW = x_origin.shape
        quant_out = self._conv_trans_1(quant_out)
        quant_out = self._conv_trans_2(quant_out)
        quant_corr = self._conv_trans_3(quant_out)
        quant_corr = quant_corr.permute(0, 2, 3, 1) \
            .reshape(N, xH * xW, C)
        '''
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)
            x, emb_loss = self.forward_features(x)
...
class TP_SwinIR(nn.Module):
    ...
    def __init__(self,
                 ...
                 binary_mask=False,
                 **kwargs):
        super(TP_SwinIR, self).__init__()
        num_in_ch = in_chans
        num_out_ch = out_chans if not out_chans == 3 else in_chans
        self.in_chans = in_chans
        num_feat = 64
        self.img_range = img_range
        if out_chans == 3 and in_chans == 3:
            rgb_mean = (0.4488, 0.4371, 0.4040)
            self.mean = torch.Tensor(rgb_mean).view(1, 3, 1, 1)
        else:
            self.mean = torch.zeros(1, 1, 1, 1)
        ...
        # benchmark.txt
        tp_num = 7936 if not binary_mask else 1
        tp_chan_out = 64
        self.tp_processor = TPBranch(tp_num, tp_chan_out)
        self.alphabet_num = tp_num
        self.fuse_conv = nn.Conv2d(embed_dim + tp_chan_out, embed_dim, 3, 1, 1)
    ...
    def forward_features(self, x, prior):
        ...
        # N, C, xH, xW = x.shape
        # print("binary-mask:", self.binary_mask)
        if self.binary_mask:
            prior_logits = self.make_binary(prior)
        else:
            prior_logits = self.make_onehot(prior)
        tp_feature = self.tp_processor(prior_logits)
        fused_feature = torch.cat([x_origin, tp_feature], dim=1)
        fused_feature = self.fuse_conv(fused_feature)
        N, C, xH, xW = x_origin.shape
        fused_feature = fused_feature.permute(0, 2, 3, 1) \
            .reshape(N, xH * xW, C)
        ...
        # decoder x +
        for layer in self.layers:
            x = layer(x + fused_feature, x_size)
    ...
    def make_onehot(self, label_map):

        # label_map: [H, W], C is set to 1
        B, C, H, W = label_map.shape
        label_map = label_map.reshape(B * C * H * W)
        label_np = label_map.data.cpu().numpy()
        # print("label_np:", label_np.shape, np.unique(label_np))
        onehot_map = nn.functional.one_hot(label_map.to(torch.int64), num_classes=self.alphabet_num)
        onehot_map = onehot_map.reshape(B, H, W, self.alphabet_num)

        return onehot_map.permute(0, 3, 1, 2).float()
    ...
    def forward(self, x, prior):
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)
            x = self.forward_features(x, prior)
            x = self.conv_after_body(x) + x
            x = self.conv_before_upsample(x)
            x = self.conv_last(self.upsample(x))
        elif self.upsampler == 'pixelshuffledirect':
