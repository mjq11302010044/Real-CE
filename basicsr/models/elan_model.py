...
class ELANModel(SRModel):

    def test(self):
        # print("SwinIR testing...")
        # pad to multiplication of window_size
        window_size = self.opt['network_g']['window_sizes'][-1]
    ...
    def tile_process_parallel(self, img, model, tile_size=54, tile_pad=5):
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                xr_shift = input_tile.shape[
                               -1] + xl_shift if input_end_x + tile_pad > width else tile_size + tile_pad * 2
                yr_shift = input_tile.shape[
                               -2] + yl_shift if input_end_y + tile_pad > height else tile_size + tile_pad * 2

                decro_input_tile = torch.zeros((batch, channel, tile_size + tile_pad * 2, tile_size + tile_pad * 2)).to(
                    input_tile.device)
        ...
        for patch_i in range(sb_num):
            ...
            decro_output_tile = model(patches, self.mask)
