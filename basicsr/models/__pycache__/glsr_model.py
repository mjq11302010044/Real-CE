...
class GLSRModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('perceptual_opt_local'):
            self.perceptual_opt_local = build_loss(train_opt['perceptual_opt_local']).to(self.device)
        else:
            self.perceptual_opt_local = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.loss_aux:
            ...
            if not (self.mask is None):
                self.outputs = self.net_g(self.lq, self.mask)
            else:
                self.outputs = self.net_g(self.lq)

        self.outputs, self.cropped_srs, self.cropped_xs, self.cropped_ys = self.outputs
        ...
        if self.cri_pix_local:
            l_pix_local = self.cri_pix(self.cropped_srs, self.cropped_ys)
            l_total += l_pix_local
        ...
        if self.perceptual_opt_local:
            l_percep_local, l_style_local = self.perceptual_opt_local(self.cropped_srs, self.cropped_ys)
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                if not self.label_mask is None:
                    self.output = self.net_g_ema(self.lq, self.mask)
                else:
