...
class TP_SwinIRwGANModel(SRGANModel): # SRGAN
    ...
    def test(self):
        ...
        # print("shape:", self.lq.shape, self.label_mask.shape)
        label_mask = F.pad(self.label_mask, (0, mod_pad_w, 0, mod_pad_h), 'reflect')

        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                # self.output = self.net_g_ema(img)
                self.output = self.tile_process_parallel(img, label_mask, self.net_g_ema)
        else:
            self.net_g.eval()
            with torch.no_grad():
                # self.output = self.net_g(img)
                self.output = self.tile_process_parallel(img, label_mask, self.net_g)
            self.net_g.train()

        _, _, h, w = self.output.size()
        self.output = self.output[:, :, 0:h - mod_pad_h * scale, 0:w - mod_pad_w * scale]

    def tile_process_parallel(self, img, label_mask, model, tile_size=54, tile_pad=5):
        ...
        for patch_i in range(sb_num):
            ...
            decro_output_tile = model(patches, masks)
...
@MODEL_REGISTRY.register()
class TP_SwinIRModel(SRModel): # SRGAN
